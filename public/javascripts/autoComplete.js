...
neverModules.modules.autocomplete.prototype = {
  ...
  _hdleClick: function (oElement) {
	...
	var urllink = this.getSelectedValue(oElement).title;
    this._inputValue = text;
    this.returnBox.value = text;
    this._fireCallbackHdle(text , content , urllink );
  ...
  _queryMutiDataSource: function (strQuery) {
    ...
    var text, content, urllink, rText, rg; this._formatData=[];
    ...
    for (var i=0; i<a.length; i++) {
      ...
      urllink=a[i].urllink?a[i].urllink:"";
      ...
      if (text.match(rg)) {
        ...
        this._formatData.push({text:text, content:content, urllink:urllink, rText: rText});
      }
    }
    /* reset cache data */
    this._cacheData = [];
  ...
  _formatMutiDataSource: function () {
    ...
    var text,rText,content,urllink,strHTML;

    for (var i=0; i<a.length; i++) {
      rText=a[i].rText?a[i].rText:a[i].text;
      text=this._encode(a[i].text?a[i].text:"");
      content=this._encode(a[i].content?a[i].content:"");
      urllink=this._encode(a[i].urllink?a[i].urllink:"");
      rText=this._decode(rText);
      strBuilder.push(''
      + '\n<tr title="' +urllink+ '" uid="' +i+ '" class="out" txt="' +text+ '" content="' +content+ '" onclick="' +this.name+'._hc(this)" onmouseover="' +this.name+'._tg(this,\'over\',\'autot-over\',\'autoc-over\')" onmouseout="' +this.name+'._tg(this,\'out\',\'autot\',\'autoc\')">\n<td class="autot">' +rText+ '</td>\n<td class="autoc">' +content+ '</td>\n</tr>\n');
    }
 ...
 show: function () {
  ...
  getSelectedValue: function (oElement) {
    ...
    var text = this.useContent? oElement.getAttribute("txt"):  oElement.innerHTML.replace(/<[^<>]*>/g,"");
    var content = this.useContent? oElement.getAttribute("content"):"";
    var title = this.useContent? oElement.getAttribute("title"):"";
    return {text: text, content: content , title : title};
  },
  ...
  hdleEvent: function (evt) {
	...
	//注：此两id为页面内容:button_sarch_xh提交按钮，span_text_xh提示框（span）
	document.getElementById('button_sarch_xh').disabled = true;
	document.getElementById("span_text_xh").innerHTML = "请点击选择列表中内容，然后查找。";
